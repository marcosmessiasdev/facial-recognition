...
namespace FaceAttributes
{
    ...
    public sealed class GenderAgeClassifier : IDisposable
    {
        ...
        {
            if (frameBgr.Empty()) return null;

            using var aligned = AlignFace(frameBgr, faceBox, _inputSize);
            ...
            DenseTensor<float> tensor = _nhwc
                ? new DenseTensor<float>(new[] { 1, _inputSize, _inputSize, 3 })
                : new DenseTensor<float>(new[] { 1, 3, _inputSize, _inputSize });
        ...
        private static (string inputName, int inputSize, bool nhwc) SelectImageInput(IReadOnlyDictionary<string, NodeMetadata> inputs)
        {
            ...
            {
                var meta = kvp.Value;
                if (meta.ElementType != typeof(float)) continue;
            ...
            if (inputs.Count == 1)
            {
                var only = inputs.First();
                var dims = only.Value.Dimensions;
        ...
        private static string SelectOutput(IReadOnlyDictionary<string, NodeMetadata> outputs)
        {
            foreach (var kvp in outputs.OrderBy(k => k.Key, StringComparer.Ordinal))
            {
                ...
                if (meta.ElementType != typeof(float)) continue;
                var dims = meta.Dimensions;
                if (dims.Length == 2 && (dims[1] == 3 || dims[1] <= 0)) return kvp.Key;
            }
