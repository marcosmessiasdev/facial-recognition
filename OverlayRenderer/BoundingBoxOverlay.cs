...
namespace OverlayRenderer
{
    ...
    /// </remarks>
    public class BoundingBoxOverlay : Window
    {
        ...
        public void UpdateTracks(List<Track> tracks, int windowWidth, int windowHeight, int overlayLeft, int overlayTop, string? hudText = null)
        {
            ...
            foreach (var track in tracks)
            {
                ...
                // Label: Name + Emotion
                string label = "";
                if (!string.IsNullOrEmpty(track.PersonName))   label += track.PersonName;
                if (track.IsSpeaking) label += "  [Speaking]";
                if (track.SpeakingScore > 0.0001f) label += $"  [SpeakScore {track.SpeakingScore:P0}]";
                if (!string.IsNullOrEmpty(track.EmotionLabel)) label += $"  [{track.EmotionLabel}]";
                if (!string.IsNullOrEmpty(track.GenderLabel))  label += $"  [{track.GenderLabel}]";
                if (!string.IsNullOrEmpty(track.AgeLabel))     label += $"  [{track.AgeLabel}]";
                if (string.IsNullOrEmpty(label)) label = $"#{track.Id}  {box.Confidence:P0}";
