    ...
    public sealed class GraphicsCaptureService : IDisposable
    {
        ...
        private void OnFrameArrived(Direct3D11CaptureFramePool sender, object args)
        {
            try
            {
                ...
                var vorticeDevice = _vorticeDevice;
                var vorticeContext = _vorticeContext;
                var device = _device;
                if (vorticeDevice == null || vorticeContext == null || device == null) return;
                ...
                try
                {
                    ...
                    using var stagingTexture = vorticeDevice.CreateTexture2D(desc);
                    vorticeContext.CopyResource(stagingTexture, texture);

                    var mappedResource = vorticeContext.Map(stagingTexture, 0, MapMode.Read, Vortice.Direct3D11.MapFlags.None);
                    try
                    {
        ...
        {
            if (_isDisposed)
                throw new ObjectDisposedException(nameof(GraphicsCaptureService));
        }
        ...
        private static IntPtr GetDXGIPointer(object winrtObject, Guid targetIid)
        {
            ...
            var dxgiAccessIid = IID_IDirect3DDxgiInterfaceAccess;
        ...
        private static GraphicsCaptureItem? CreateItemForWindow(IntPtr hwnd)
        {
            ...
            try
            {
                ...
                WindowsCreateString(runtimeClassId, runtimeClassId.Length, out hString);

                int hr = RoGetActivationFactory(hString, ref iidInterop, out factoryPtr);
                ...
                var captureItem = GraphicsCaptureItem.FromAbi(rawItemPtr);
                return captureItem;
            }
            catch
            {
