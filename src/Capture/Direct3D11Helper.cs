...
{
    [LibraryImport("d3d11.dll", EntryPoint = "D3D11CreateDevice", SetLastError = true)]
    ...
    /// <exception cref="InvalidOperationException">Thrown if the device creation or interface query fails.</exception>
    public static IDirect3DDevice CreateDevice()
    {
        ...
        if (hr != 0)
        {
            throw new InvalidOperationException($"Failed to create D3D11 device. HRESULT 0x{hr:X8}");
        }

        Guid dxgiDeviceGuid = new("54ec77fa-1377-44e6-8c32-88fd5f44c84c"); // IDXGIDevice
        hr = Marshal.QueryInterface(d3dDevice, ref dxgiDeviceGuid, out IntPtr dxgiDevice);

        if (hr != 0)
        {
            _ = Marshal.Release(d3dDevice);
            throw new InvalidOperationException($"Failed to query IDXGIDevice. HRESULT 0x{hr:X8}");
        }
        ...
        {
            throw new InvalidOperationException($"Failed to create WinRT Direct3D11 device from DXGI device. HRESULT 0x{hr:X8}");
        }

        IDirect3DDevice device = WinRT.MarshalInspectable<IDirect3DDevice>.FromAbi(inspectableDevice);
