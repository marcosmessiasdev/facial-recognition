...
public sealed class StreamingWhisperPipeline : IDisposable
{
    ...
    private void OnSegmentReady(object? sender, (TimeSpan Start, TimeSpan End, float[] Samples) seg)
    {
        ...
        Task task = Task.Run(async () =>
        {
            ...
            try
            {
                ...
                using CancellationTokenSource linked = CancellationTokenSource.CreateLinkedTokenSource(_shutdownCts.Token, timeoutCts.Token);
                string text = await _transcriber.TranscribeAsync(seg.Samples, _language, linked.Token);
    ...
    public void Dispose()
    {
        _stopping = true;
        _segmenter.SegmentReady -= OnSegmentReady;
        try { _segmenter.Stop(); } catch { /* ignore */ }
        ...
        // Ensure no Whisper native state is disposed while background tasks still reference it.
        if (tasks.Length > 0)
        {
            try { _ = Task.WaitAll(tasks, TimeSpan.FromSeconds(5)); } catch { /* ignore */ }
        }
