...
public class FaceDetector : IDisposable
{
    ...
    public FaceDetector(string modelPath)
    {
        ...
        _detector = new ScrfdDetector(_cache, new ScrfdDetectorOptions { ModelPath = modelPath });
    }
    ...
    /// 3. Loads the data into a <see cref="Image"/> for model ingestion.
    ...
    public List<BoundingBox> Detect(VisionFrame frame)
    {
        ...
        IReadOnlyCollection<FaceDetectorResult> faces = _detector.DetectFaces(image);

        foreach (FaceDetectorResult face in faces)
        {
            ...
            boxes.Add(new BoundingBox
            {
                ...
                Landmarks = face.Landmarks != null
                    ? [.. face.Landmarks.Select(p => new System.Drawing.PointF(p.X, p.Y))]
                    : null
            });
        }
        return boxes;
