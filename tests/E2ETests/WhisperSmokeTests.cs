...
public sealed class WhisperSmokeTests
{
    private static string FindRepoRoot(string startDir)
    {
        ...
        while (cur != null)
        {
            if (Directory.Exists(Path.Combine(cur.FullName, "src", "App", "models", "whisper")))
            {
                return cur.FullName;
            }
            cur = cur.Parent;
        ...
        throw new DirectoryNotFoundException("Repo root not found (expected 'src/App/models/whisper' folder).");
    }
...
#pragma warning restore CA1707
    {
        ...
        string modelPath = Path.Combine(repoRoot, "src", "App", "models", "whisper", "ggml-tiny.bin");
